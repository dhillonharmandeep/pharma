...
/* The variable storing the input field */
var oKeyword;
...
function init(oInput, ajaxFile, secondArgument) 
{ 
  // Re-set only if any of the arguments have changed
  if(oKeyword !=  oInput || getFunctionsUrl != ajaxFile || optArg2 != secondArgument) {
      ...
      { 
        try
        {
          ...
          {    
            ...
            if(optArg2){
            	xmlHttpGetSuggestions.open("GET", getFunctionsUrl + encode(keyword) + '/' + optArg2, true);
            }
            else{
...
{  
  ...
  if(results_array.length == 0)
  {
    div += "<tr><td>No results found for <strong>" + keyword + 
           "</strong>. A new chain/banner group by this name will be created</td></tr>";
  ...
  else
  {
    ...
    {  
      ...
      div += "<tr id='tr" + i + 
             "' onclick='handleOnClick(this);return false;' onmouseover='handleOnMouseOver(this);' " + 
  ...
  oScroll.style.left = (getX(oKeyword)-225) +"px";
  oScroll.style.top = (getY(oKeyword) + oKeyword.offsetHeight -105) +"px";
  // harman's code ends
...
//  var oKeyword = document.getElementById("keyword");
  ...
  var crtLink = document.getElementById("a" + 
                            oTr.id.substring(2,oTr.id.length)).innerHTML;//.toString();
//  alert(crtLink.innerHTML);
  // replace - with _ and leave out the .php extension
 
//  crtLink = crtLink.replace("-", "_");
  crtLink = crtLink.replace(/<b>/i, "");
  crtLink = crtLink.replace(/<\/b>/i, "");
//  alert(crtLink);
